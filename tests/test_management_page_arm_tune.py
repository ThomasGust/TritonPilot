...
def test_arm_tuning_controls_load_and_save_rov_config(monkeypatch):
    ...
    try:
        page._apply_state(
            {
                ...
                "config": {
                    ...
                    "GRIPPER_PITCH_NEUTRAL_DEG": 70.0,
                },
            }
        )
        ...
        assert page._arm_tune_spins["servo_range_deg"].value() == 70.0
        assert page._arm_tune_spins["pitch_span_deg"].value() == 90.0
        assert page._arm_tune_spins["pitch_neutral_deg"].value() == 70.0
        ...
        assert args["updates"]["GRIPPER_PITCH_NEUTRAL_DEG"] == 70.0
    finally:
        page.shutdown()
        page.close()
        app.processEvents()
